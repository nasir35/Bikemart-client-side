...
const Product = ({product}) => {
    ...
    const handleDelete = id => {
        ...
        if (proceed){
            const url = `https://frozen-scrubland-17701.herokuapp.com/products/${id}`;
            fetch(url, {
                method : 'DELETE'
            })
            .then(res => res.json())
    ...
    return (
        <div className="p-3 border rounded shadow-inset bg-white space-y-2">
            ...
            <div className="flex justify-between sm:text-base text-sm">
                ...
                <button className="px-5 py-1 bg-green-custom text-white rounded" onClick={() => handleDelete(product._id)}><i className="far fa-paper-plane"></i> Delete</button>
            </div>
        </div>
    );
};
