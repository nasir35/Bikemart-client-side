...
const Reviews = () => {
  ...
  useEffect(() => {
    fetch(
      "https://bikemart-server-side-bem11l6mg-nasir35s-projects.vercel.app/reviews/published"
    )
      .then((res) => res.json())
    ...
    <div className="text-center bg-gray-100 pt-5 pb-8">
      ...
      {reviews.length === 0 ? (
        ...
        <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 md:p-12 md:pb-4 sm:p-10 sm:pb-3 pb-2 p-5 gap-8">
          {reviews.map((review) => (
            <Review reviewObj={review} key={review._id}></Review>
          ))}
        </div>
      )}
      <Link to="dashboard/review">
        <button className="mx-auto block bg-coral hover:bg-yellow-600 text-white px-10 py-1 rounded">
          Give a Review
        </button>
      </Link>
    </div>
