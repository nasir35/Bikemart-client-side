...
const ProductDetails = () => {
  ...
  useEffect(() => {
    fetch(
      `https://bikemart-server-side-nasir35.vercel.app/products/${productId}`
    )
      .then((res) => res.json())
  ...
  const alreadyTaken = (idToCheck) => {
    fetch(`https://bikemart-server-side-nasir35.vercel.app/orders/${idToCheck}`)
      .then((res) => res.json())
      .then((data) => {
        if (data.uniqueId) {
          return true;
  ...
  const onSubmit = (data) => {
    ...
    console.log(data);
    fetch(
      "https://bikemart-server-side-bem11l6mg-nasir35s-projects.vercel.app/placeorder",
      {
        method: "POST",
        headers: {
      ...
      <div className="md:p-10 sm:p-6 p-3">
        {selectedProduct.length == 0 ? (
          ...
          <div id="wrapper-div" className="grid grid-cols-5 gap-5">
            <div
              id="product-details"
              className="md:col-span-3 col-span-5 space-y-3"
            >
              ...
              >
                ...
                <div className="w-full grid grid-cols-12 mb-3 md:px-4 px-2">
                  ...
                  <input
                    {...register("bikeType")}
                    id="bikeType"
                    className="col-span-9 mb-1 bg-indigo-100 px-3 py-1 rounded"
